...
public class tk2dButton : MonoBehaviour 
{
	...
	IEnumerator coHandleButtonPress(int fingerId)
	{
        ...
        if (targetObject) {
            if (!ActionDown.Equals(""))
                targetObject.SendMessage(ActionDown);
		...
		if (buttonPressed)
		{
			...
			if (targetObject)
			{
                ...
                {
                    if (!ActionUp.Equals(""))
                        targetObject.SendMessage(ActionUp);

                }
                else
                {
                    if (!ActionUp.Equals(""))
                        targetObject.SendMessage(ActionUp, itemNumber);
